...
client.on("ready", () => {
  ...
  client.user.setActivity("YouTube.com/@VitkTeam", {
    type: "PLAYING",
...
console.log(``)
 ...
 console.log(`Bot Tag : ${client.user.tag}`)
  ...
  console.log(`Users Count : ${client.guilds.cache .reduce((a, b) => a + b.memberCount, 0)
 .toLocaleString()}`)
console.log(``)
   console.log(`================`)
 });

const developer = "982639214963331092";
const developer1 = "919670443273768991";
const developer2 = "571010038945153039";
const developer3 = "597470143211372567";
...
let command = message.content.toLowerCase().split(" ")[0];
  if (command == prefix + `اسهم`) {
    ...
    .setThumbnail(`https://cdn2.iconfinder.com/data/icons/webina-seo-development-and-marketing/128/seo_web-91-512.png`)
          ...
          .addField(`**\`5\`** : **\`تكون اونر بسيرفر برلين\`**`, `السعر: **\`${zydh}\`**`, true)
          .addField(`**\`6\`** : **\`شركة جراح\`**`, `السعر: **\`${jra7}\`**`, true)
          .addField(`**\`7\`** : **\`نايف للإتصالات\`**`, `السعر: **\`${khaled}\`**`, true)
          .addField(`**\`8\`** : **\`ديفرزو للنصب\`**`, `السعر: **\`${diverso}\`**`, true)
  ...
  if (command == prefix + `اسهمي`) {
    ...
    if (!ashm[message.guild.id][user.id]) ashm[message.guild.id][user.id] = {
     ...
     .setColor(`2f3135`)
          ...
          .addField(`**\`5\`** : **\`اونر برلين\`**`, `سهم: **\`${ashm5}\`**`, true)
          .addField(`**\`6\`** : **\`شركة جراح\`**`, `سهم: **\`${ashm6}\`**`, true)
          .addField(`**\`7\`** : **\`نايف للإتصالات\`**`, `سهم: **\`${ashm7}\`**`, true)
          .addField(`**\`8\`** : **\`ديفرزو للنصب\`**`, `سهم: **\`${ashm8}\`**`, true)
...
const cooldown8mar = new Map();
client.on("message", async message =>{
    if(message.author.bot) return
    if(!message.guild) return
let command = message.content.toLowerCase().split(" ")[0];
  if (command == prefix + `قمار`) {
      var blacklist = db.get(`Blacklist_${message.author.id}`)
    if (blacklist) return
    let args = message.content.toLowerCase().split(" ")[1];
      var author = message.author.id
    let member = credits[message.guild.id][author].credits
      ...
      var ffb = -args
      var ff = args / 1.5
let amount2 = Math.floor(Math.random() * ff) + 1;

let demo = [`${-args}`, `${amount2}`];
let amount = demo[Math.floor(Math.random() * demo.length)];
var ddb = member - t7
{const cooldownTime = ms('5m');
  const cooldownUser = cooldown8mar.get(message.author.id);
  if(cooldownUser) {
    let remaining = Duration(cooldownUser - Date.now(), { units: ["w", "d", "h", "m", "s"], round: true });
...
**\`${command} 500\`**`)
    ...
    message.lineReply(`> **\`${t7}$\`** تم سحب \nالسبب: محاولة غسيل اموال`)
      db.add(`t7_${message.author.id}` -aa)
    } else {

     if(args < 1000) return message.lineReply(`**\`أقل مبلغ هو 1000\`**`)
...
var nsbh = Math.round(number * 10) / 10;
      let bal2 = credits[message.guild.id][author].credits
    credits[message.guild.id][author].credits += +amount;
      ...
      const embed1 = new Discord.MessageEmbed()
          .setAuthor(message.guild.name)
      .setTitle(`قمار خاسر`)
.setThumbnail(`https://cdn.discordapp.com/attachments/911030299230625842/974620769134968893/jackpot-machine.png`)
      .setDescription(`قامرت بـ: ${args} وخسرتهم
رصيدك السابق: **${bal2}$**
رصيدك الحالي: **${bal}$**`)
      .setFooter(message.guild.name, message.guild.iconURL())
...
.setTitle(`قمار رابح`)
.setThumbnail(`https://cdn.discordapp.com/attachments/911030299230625842/974620769134968893/jackpot-machine.png`)
      .setDescription(`قامرت بـ: ${args} وربحت ${alls}
رصيدك السابق: **${bal2}$**
رصيدك الحالي: **${bal}$**`)
      .setFooter(message.guild.name, message.guild.iconURL())
message.lineReply(embed2)
      cooldown8mar.set(message.author.id, Date.now() + cooldownTime);
      save();
      setTimeout(() => {
          cooldown8mar.delete(message.author.id)
      }, cooldownTime);
  }}
}}});
...
var zwaj = require("./zwaj.json");
function zwajs() {
    fs.writeFileSync("./zwaj.json", JSON.stringify(zwaj, null, 4));
}

var topzwaj = require("./topzwaj.json");
function topss() {
    fs.writeFileSync("./topzwaj.json", JSON.stringify(topzwaj, null, 4));
}
...
client.on("message", async message =>{
    if(message.author.bot) return
    if(!message.guild) return
        if (!zwaj[message.guild.id]) zwaj[message.guild.id] = {};
    if (!zwaj[message.guild.id][message.author.id]) zwaj[message.guild.id][message.author.id] = {
        "zwj": "0",
        "zwjh": "0",
        "mhr": 0,
        "time": "0",
        "id": "0",
            ...
            if (!topzwaj[message.guild.id]) topzwaj[message.guild.id] = {};
    if (!topzwaj[message.guild.id][message.author.id]) topzwaj[message.guild.id][message.author.id] = {
        "zwjh": "0",
        "mhr": 0,
        "id": "0",
    }
...
**\`${command} ${name} 100000\`**`)
                if (!zwaj[message.guild.id]) zwaj[message.guild.id] = {};
    if (!zwaj[message.guild.id][user.id]) zwaj[message.guild.id][user.id] = {
        "zwj": "0",
        "zwjh": "0",
        "mhr": 0,
        "time": "0",
   ...
   if(user.id === client.user.id) return message.lineReply(`> صاحي انت؟ تتزوج بوت`)
    if (member < args) return message.lineReply(`> **المبلغ اكثر من رصيدك**`)
    if(message.author.id === user.id) return message.lineReply(`> ماتقدر تتزوج نفسك`)
    let datas = zwaj[message.guild.id][user.id].id
    if (user.id === datas) return message.lineReply("> **هذا الشخص متزوج**")
    var dd = zwaj[message.guild.id][message.author.id].id
    if(message.author.id === dd) return message.lineReply(`> **التعدد ممنوع**`)
    if(args < 100000) return message.lineReply(`الزواجات عندنا من 100 الاف وطالع!`)
  const button = new disbut.MessageButton().setID('1').setLabel('موافقه').setStyle('green')
  const button2 = new disbut.MessageButton().setID('2').setLabel('غير موافقه').setStyle('red')
  const butto = new disbut.MessageButton().setID('1.').setLabel('موافقه').setStyle('green').setDisabled()
  const butto2 = new disbut.MessageButton().setID('2.').setLabel('غير موافقه').setStyle('red').setDisabled()

  let msg = await message.channel.send(`**هل أنتِ موافقه على الزواج <@!${user.id}>**`, {buttons: [button,button2]})

  const collector = msg.createButtonCollector((button) => button.clicker.user.id === user.id, {time: 60e3})
...
alert(riyadh.toString());
 var ff = moment(riyadh).format("hh:mm a - YYYY/MM/DD");
    var author = user.id
    var amount = -args
    credits[message.guild.id][message.author.id].credits += amount
    zwaj[message.guild.id][message.author.id].zwjh = user.id
    zwaj[message.guild.id][message.author.id].id = message.author.id
    zwaj[message.guild.id][message.author.id].mhr = args
    zwaj[message.guild.id][message.author.id].time = `${ff}`;
    zwaj[message.guild.id][user.id].zwj = message.author.id
    zwaj[message.guild.id][user.id].id = user.id
    zwaj[message.guild.id][user.id].mhr = args
    zwaj[message.guild.id][user.id].time = `${ff}`;
    topzwaj[message.guild.id][message.author.id].zwjh = user.id
    topzwaj[message.guild.id][message.author.id].id = message.author.id
    topzwaj[message.guild.id][message.author.id].mhr = args
    save();
    zwajs();
    topss();
 const embed = new Discord.MessageEmbed()
.setAuthor(message.guild.name)
    .setTitle(`عقد زواج`)
     .setThumbnail(`https://cdn1.iconfinder.com/data/icons/love-wedding-vol-2/512/engagement_rings_wedding_marriage-512.png`)
    .setDescription(`> **عقد زواج**
🤵🏻 <@!${message.author.id}> | 👰🏻‍♀️ <@!${user.id}>
المهر: ${args}$
تاريخ الزواج:
**\`${ff}\`**`)
.setFooter(message.guild.name, message.guild.iconURL())
//message.lineReply(embed)
          await msg.edit(embed, {buttons : [butto,butto2]})
        }
        if(b.id === '2'){
          await msg.edit(`**ماتبيك شوف غيرها**`, {buttons : [butto,butto2]})
        } else {
        }})
}});
...
client.on('message', message => {
    ...
    if(!message.guild) return
let command = message.content.toLowerCase().split(" ")[0];
    if (command == prefix + `زواجي`) {
        let user = message.mentions.members.first() || message.author
 if(!user) return message.lineReply(`> يُرجى استعمال الأمر بالطريقة الصحيحة ، 
**\`${command} ${name}\`**`)
    var dd = zwaj[message.guild.id][user.id].id
    if(user.id !== dd) return message.lineReply(`> **هذا العضو عزابي**`)
    var ddb = zwaj[message.guild.id][user.id].zwjh
    if(ddb) zwj = zwaj[message.guild.id][user.id].zwj
    if(zwj === "0") zwj = user.id
    var ddb = zwaj[message.guild.id][user.id].zwj
    if(ddb) zwjh = zwaj[message.guild.id][user.id].zwjh
    if(zwjh === "0") zwjh = user.id
        var mhr = zwaj[message.guild.id][user.id].mhr
        var times = zwaj[message.guild.id][user.id].time
const embed = new Discord.MessageEmbed()
    .setAuthor(message.guild.name)
.setTitle(`وثيقة زواج`)
    .setThumbnail(`https://cdn1.iconfinder.com/data/icons/love-wedding-vol-2/512/engagement_rings_wedding_marriage-512.png`)
...
client.on('message', message => {
    ...
    if(!message.guild) return
let command = message.content.toLowerCase().split(" ")[0];
    if (command == prefix + `طلاق`) {
        let user = message.mentions.members.first()
 if(!user) return message.lineReply(`> يُرجى استعمال الأمر بالطريقة الصحيحة ، 
**\`${command} ${name}\`**`)
    var dd = zwaj[message.guild.id][user.id].id
    if(user.id !== dd) return message.lineReply(`لايمكنك الطلاق من غير زوجتك`)
    var ddb = zwaj[message.guild.id][user.id].zwjh
    if(ddb) zwj = zwaj[message.guild.id][user.id].zwj
    if(zwj === "0") zwj = user.id
    var ddb = zwaj[message.guild.id][user.id].zwj
    ...
    if(zwjh === "0") zwjh = user.id
        var mhr = zwaj[message.guild.id][user.id].mhr
        var times = zwaj[message.guild.id][user.id].time
        if(message.author.id === zwjh) { return message.lineReply(`**الطلاق للرجل فقط**`) }
        if(user.id !== zwjh) return message.lineReply(`لايمكنك الطلاق من غير زوجتك`)
        if(message.author.id !== zwj) return message.lineReply(`> تزوج وتعال`)
zwaj[message.guild.id][message.author.id].zwjh = "0"
    zwaj[message.guild.id][message.author.id].id = "0"
    zwaj[message.guild.id][message.author.id].mhr = 0
    zwaj[message.guild.id][message.author.id].time = "0"
    zwaj[message.guild.id][zwjh].zwj = "0"
    zwaj[message.guild.id][zwjh].id = "0"
    zwaj[message.guild.id][zwjh].mhr = 0
    zwaj[message.guild.id][zwjh].time = "0"
    topzwaj[message.guild.id][message.author.id].zwjh = "0"
    topzwaj[message.guild.id][message.author.id].mhr = 0
        topzwaj[message.guild.id][message.author.id].id = "0"
    zwajs();
    topss();
        const embed = new Discord.MessageEmbed()
        .setAuthor(message.guild.name)
        .setTitle(`صك طلاق`)
        .setThumbnail(`https://cdn3.iconfinder.com/data/icons/marriage-family-problem/341/couple-quarrel-family-problem-006-512.png`)
        .setDescription(`> **اصدار صك طلاق**
🤵🏻 <@!${zwj}> | 👰🏻‍♀️ <@!${zwjh}>`)
.setFooter(message.guild.name, message.guild.iconURL())
message.lineReply(embed)
}
...
client.on('message', message => {
    ...
    if(!message.guild) return
let command = message.content.toLowerCase().split(" ")[0];
    if (command == prefix + `خلع`) {
        let user = message.mentions.members.first()
 if(!user) return message.lineReply(`> يُرجى استعمال الأمر بالطريقة الصحيحة ، 
**\`${command} ${name}\`**`)
    if (user.id !== developer) return message.lineReply(`> خيييررر ؟؟`)
    var dd = zwaj[message.guild.id][user.id].id
    if(user.id !== dd) return message.lineReply(`لايمكنك خلع غير زوجك`)
    var ddb = zwaj[message.guild.id][user.id].zwjh
    if(ddb) zwj = zwaj[message.guild.id][user.id].zwj
    if(zwj === "0") zwj = user.id
    var ddb = zwaj[message.guild.id][user.id].zwj
    ...
    if(zwjh === "0") zwjh = user.id
        var mhr = zwaj[message.guild.id][user.id].mhr
        var times = zwaj[message.guild.id][user.id].time
        if(message.author.id === zwj) { message.lineReply(`> **الخلع للنساء فقط**`) }
        if(user.id === zwj) return message.lineReply(`لايمكنك خلع غير زوجك`)
        if(message.author.id !== zwjh) return message.lineReply(`> تزوجي وتعالي`)
zwaj[message.guild.id][zwj].zwjh = "0"
    zwaj[message.guild.id][zwj].id = "0"
    zwaj[message.guild.id][zwj].mhr = 0
    zwaj[message.guild.id][zwj].time = "0"
    zwaj[message.guild.id][message.author.id].zwj = "0"
    zwaj[message.guild.id][message.author.id].id = "0"
    zwaj[message.guild.id][message.author.id].mhr = 0
    zwaj[message.guild.id][message.author.id].time = "0"
    topzwaj[message.guild.id][zwj].zwjh = "0"
    topzwaj[message.guild.id][zwj].mhr = 0
        topzwaj[message.guild.id][zwj].id = "0"
    zwajs();
    topss();
        const embed = new Discord.MessageEmbed()
        .setAuthor(message.guild.name)
...
client.on("message", async function (message) {
    ...
    var cmd = args[0].toLowerCase();
    if (!message.content.startsWith(prefix)) return;
  if(message.content.startsWith(prefix + "زواجات")) {
            var topArray = Object.values(topzwaj[message.guild.id]);
      var num = 1;
  var sorted = Object.entries(topzwaj[message.guild.id]) 
    .map(v => [v[0], v[0].topzwaj])
    .sort((a, b) => b[0] - a[0])
      sorted.length = 10
      var textStr = `${topArray.sort((a, b) => b.mhr - a.mhr).slice(0, 10).map(function (user) {
                if (user.mhr > 0) {
                    return `**#${num++} |** 🤵<@!${user.id}> | 👰<@!${user.zwjh}> **\`${user.mhr}\`** 💰`
                }
            }).join("\n")}`;
            var embed = new Discord.MessageEmbed()
  .setColor("AAF5EF")
